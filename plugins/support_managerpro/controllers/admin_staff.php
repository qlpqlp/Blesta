 ...
 */
class AdminStaff extends SupportManagerproController {
	...
	public function preAction() {
		...
		Language::loadLang("admin_tickets", null, PLUGINDIR . "support_managerpro" . DS . "language" . DS);
	}
	...
	public function index() {
		...
		$this->set("staff", $this->SupportManagerproStaff->getList($this->company_id, null, true, $page, array($sort => $order)));
		
		// Overwrite default pagination settings
		$settings = array_merge(Configure::get("Blesta.pagination"), array(
				'total_results' => $this->SupportManagerproStaff->getListCount($this->company_id),
				'uri' => $this->base_uri . "plugin/support_managerpro/admin_staff/[p]/",
	 ...
	 */
	public function add() {
		$this->uses(array("StaffGroups", "SupportManagerpro.SupportManagerproDepartments"));
		
		// Get all available departments
		$departments = $this->Form->collapseObjectArray($this->SupportManagerproDepartments->getAll($this->company_id), "name", "id");
		...
		if (!empty($this->post)) {
			...
			$this->SupportManagerproStaff->add($data);
			
			if (($errors = $this->SupportManagerproStaff->errors())) {
				// Error, reset vars
				...
				$this->redirect($this->base_uri . "plugin/support_managerpro/admin_staff/");
			}
		}
		...
		$times = array('' => Language::_("AdminStaff.text.no_time", true)) + $this->SupportManagerproStaff->getTimes();
		...
		$this->set("days", $this->SupportManagerproStaff->getDays(true));
	...
	public function edit() {
		...
		if (!isset($this->get[0]) || !($staff = $this->SupportManagerproStaff->get($this->get[0], $this->company_id)) ||
		...
		if (!empty($this->post)) {
			...
			$this->SupportManagerproStaff->edit($staff->id, $data);
			...
			foreach ($days as $day => $lang) {
				...
				foreach ($schedules as &$schedule) {
					...
					if ($day == $schedule->day) {
						...
						// Format the time
						$day_schedule['start_time'] = $this->Date->cast($day_schedule['start_time'], Configure::get("SupportManagerpro.time_format"));
						$day_schedule['end_time'] = $this->Date->cast($day_schedule['end_time'], Configure::get("SupportManagerpro.time_format"));
						break;
					}
				}
	...
	public function delete() {
		...
		// Delete the staff
		$this->SupportManagerproStaff->delete($staff->id, $this->company_id);
	...
	public function getSchedule() {
		...
		if (!$this->isAjax() || !isset($this->get[0]) || !($staff = $this->SupportManagerproStaff->get($this->get[0], $this->company_id))) {
	...
	private function getStaffList($staff_group_id, $company_id) {
		...
		$existing_staff = $this->SupportManagerproStaff->getAll($company_id);
