				...
				if ($ticket) {
					...
					if (!$reply_id) {
						...
						if ($this->SupportManagerproTickets->checkLoopBack($email, $this->max_reply_limit, $this->reply_period)) {
							...
							$additional_tags = array('SupportManagerpro.ticket_updated' => array('update_ticket_url' => $this->Html->safe($hostname . $client_uri . "plugin/support_managerpro/client_tickets/reply/" . $ticket->id . "/")));
		...
		// Handle creating a new ticket
			...
			foreach ($to as $address) {
				...
				if ($department) {
					...
					if ($this->SupportManagerproTickets->checkLoopBack($email, $this->max_reply_limit, $this->reply_period)) {
						// Send the email associated with this ticket
						$additional_tags = array('SupportManagerpro.ticket_updated' => array('update_ticket_url' => $this->Html->safe($hostname . $client_uri . "plugin/support_managerpro/client_tickets/reply/" . $ticket_id . "/")));
						$this->SupportManagerproTickets->sendEmail($reply_id, $additional_tags);
					}
					return;
				}
