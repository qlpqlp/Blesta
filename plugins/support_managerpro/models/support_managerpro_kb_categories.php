<?php
/**
 * SupportManagerproKbCategories model
 * 
 * @package blesta
 ...
 */
class SupportManagerproKbCategories extends SupportManagerproModel {
	...
	public function __construct() {
		...
		Language::loadLang("support_managerpro_kb_categories", null, PLUGINDIR . "support_managerpro" . DS . "language" . DS);
	}
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->insert("support_kb_categoriespro", $vars, $fields);
	...
	public function edit($category_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->where("id", "=", $category_id)->update("support_kb_categoriespro", $vars, $fields);
	...
	public function delete($category_id) {
		...
		$rules = array(
			'category_id' => array(
				'parent' => array(
					...
					'message' => $this->_("SupportManagerproKbCategories.!error.articles.parent")
				)
			)
		);
		...
		if ($this->Input->validates($vars) && ($category = $this->get($category_id))) {
			...
			$this->Record->where("parent_id", "=", $category_id)->
				update("support_kb_categoriespro", array('parent_id' => $category->parent_id));
			
			// Remove articles assigned to this category along with this category
			$this->Record->from("support_kb_categoriespro")->
				leftJoin("support_kb_article_categoriespro", "support_kb_article_categoriespro.category_id", "=", "support_kb_categoriespro.id", false)->
				where("support_kb_categoriespro.id", "=", $category_id)->
				delete(array("support_kb_categoriespro.*", "support_kb_article_categoriespro.*"));
	...
	public function getItemCount($category_id, $access = null) {
		...
		$this->Record->select(array("support_kb_articlespro.id"))->from("support_kb_articlespro")->
		...
		$this->Record->select(array("support_kb_categoriespro.id"))->from("support_kb_categoriespro")->
			where("support_kb_categoriespro.parent_id", "=", $category_id);
		
		// Filter by access level
		if ($access !== null && is_array($access))
			$this->Record->where("support_kb_categoriespro.access", "in", array_values($access));
	...
	private function getCategories($category_id = null, $company_id = null) {
		$this->Record->select()->from("support_kb_categoriespro");
	...
	public function getAccessTypes() {
		return array(
			'public' => $this->_("SupportManagerproKbCategories.access_types.public"),
			'private' => $this->_("SupportManagerproKbCategories.access_types.private"),
			'hidden' => $this->_("SupportManagerproKbCategories.access_types.hidden")
		);
	}
	...
	private function getRules(array $vars, $edit = false) {
		$rules = array(
			'parent_id' => array(
				'exists' => array(
					...
					'message' => $this->_("SupportManagerproKbCategories.!error.parent_id.exists")
				),
				'valid_company' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateCompanyMatches"), $this->ifSet($vars['company_id'])),
					'message' => $this->_("SupportManagerproKbCategories.!error.parent_id.valid_company")
				),
				'valid_parent' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateParent"), $this->ifSet($vars['category_id'])),
					'message' => $this->_("SupportManagerproKbCategories.!error.parent_id.valid_parent")
				)
			),
			'company_id' => array(
				'exists' => array(
					'rule' => array(array($this, "validateExists"), "id", "companies"),
					'message' => $this->_("SupportManagerproKbCategories.!error.company_id.exists")
				)
			),
			'name' => array(
				'empty' => array(
					'rule' => "isEmpty",
					'negate' => true,
					'message' => $this->_("SupportManagerproKbCategories.!error.name.empty")
				),
				'length' => array(
					'rule' => array("maxLength", 255),
					'message' => $this->_("SupportManagerproKbCategories.!error.name.length")
				)
			),
			'access' => array(
				'type' => array(
					...
					'message' => $this->_("SupportManagerproKbCategories.!error.access.type")
				)
			),
			'date_created' => array(
				'valid' => array(
		...
		if ($edit) {
			...
			$rules['category_id'] = array(
				'exists' => array(
					'rule' => array(array($this, "validateExists"), "id", "support_kb_categoriespro"),
					'message' => $this->_("SupportManagerproKbCategories.!error.category_id.exists")
				)
			);
		}
	...
	public function validateCategoryDeletion($category_id) {
		...
		$this->Record->select(array("support_kb_article_categoriespro.article_id"))->
			from("support_kb_article_categoriespro")->
			where("support_kb_article_categoriespro.category_id", "=", $category_id);
		...
		$count = $this->Record->select(array("support_kb_article_categoriespro.category_id"))->
			from("support_kb_article_categoriespro")->
			appendValues($values)->
			innerJoin(array($sub_query => "articlespro"), "articlespro.article_id", "=", "support_kb_article_categoriespro.article_id", false)->
			where("support_kb_article_categoriespro.category_id", "!=", $category_id)->
			group(array("articlespro.article_id"))->numResults();
