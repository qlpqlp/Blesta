<?php
/**
 * SupportManagerproResponses model
 * 
 * @package blesta
 ...
 */
class SupportManagerproResponses extends SupportManagerproModel {
	...
	public function __construct() {
		...
		Language::loadLang("support_managerpro_responses", null, PLUGINDIR . "support_managerpro" . DS . "language" . DS);
	}
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			// Add the response
			$fields = array("category_id", "name", "details");
			$this->Record->insert("support_responsespro", $vars, $fields);
	...
	public function edit($response_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->where("id", "=", $response_id)->
				update("support_responsespro", $vars, $fields);
	...
	public function delete($response_id) {
		$this->Record->from("support_responsespro")->
	...
	public function get($response_id) {
		$fields = array("support_responsespro.*", "support_response_categoriespro.company_id");
		return $this->Record->select($fields)->from("support_responsespro")->
			innerJoin("support_response_categoriespro", "support_response_categoriespro.id", "=", "support_responsespro.category_id", false)->
			where("support_responsespro.id", "=", $response_id)->
			fetch();
	}
	...
	public function getAll($company_id, $category_id) {
		...
		return $this->Record->select($fields)->
			from("support_responsespro")->
			innerJoin("support_response_categoriespro", "support_response_categoriespro.id", "=", "support_responsespro.category_id", false)->
			where("support_response_categoriespro.company_id", "=", $company_id)->
			where("support_responsespro.category_id", "=", $category_id)->
			order(array('support_responsespro.name' => "ASC"))->
			fetchAll();
	}
	...
	public function addCategory(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			$fields = array("company_id", "parent_id", "name");
			$this->Record->insert("support_response_categoriespro", $vars, $fields);
	...
	public function editCategory($category_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->where("id", "=", $category_id)->
				update("support_response_categoriespro", $vars, $fields);
	...
	public function deleteCategory($category_id) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->where("parent_id", "=", $category->id)->
				update("support_response_categoriespro", array('parent_id'=>$category->parent_id));
			
			// Update responses in this category to be in the parent category
			$this->Record->where("category_id", "=", $category->id)->
				update("support_responsespro", array('category_id'=>$category->parent_id));
			
			// Finally, delete this category
			$this->Record->from("support_response_categoriespro")->
	...
	public function getCategory($category_id) {
		return $this->Record->select()->from("support_response_categoriespro")->
	...
	public function validateCategoryCompany($category_id, $company_id) {
		$count = $this->Record->select()->from("support_response_categoriespro")->
	 ...
	 *
	 * @param int $category_id The ID of the category to delete
	 * @return boolean True if the category can be deleted, false otherwise
	 */
	public function validateDeleteCategory($category_id) {
		...
		$count = $this->Record->select(array("support_responsespro.id"))->
			from("support_responsespro")->
			innerJoin("support_response_categoriespro", "support_response_categoriespro.id", "=", "support_responsespro.category_id", false)->
			where("support_responsespro.category_id", "=", $category_id)->
			where("support_response_categoriespro.parent_id", "=", null)->
			numResults();
	...
	private function getCategoryDeleteRules() {
		$rules = array(
			'category_id' => array(
				'exists' => array(
					'rule' => array(array($this, "validateExists"), "id", "support_response_categoriespro"),
					'message' => $this->_("SupportManagerproResponses.!error.category_id.exists")
				),
				'root_responses' => array(
					'rule' => array(array($this, "validateDeleteCategory")),
					'message' => $this->_("SupportManagerproResponses.!error.category_id.root_responses")
				)
			)
		);
		...
		$rules = array(
			'company_id' => array(
				'exists' => array(
					...
					'message' => $this->_("SupportManagerproResponses.!error.company_id.exists")
				)
			),
			'parent_id' => array(
				'exists' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateExists"), "id", "support_response_categoriespro"),
					'message' => $this->_("SupportManagerproResponses.!error.parent_id.exists")
				),
				'company' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateCategoryCompany"), $this->ifSet($vars['company_id'])),
					'message' => $this->_("SupportManagerproResponses.!error.parent_id.company")
				)
			),
			'name'=> array(
				'empty' => array(
					'rule' => "isEmpty",
					'negate' => true,
					'message' => $this->_("SupportManagerproResponses.!error.name.empty")
				),
				'length' => array(
					'rule' => array("maxLength", 64),
					'message' => $this->_("SupportManagerproResponses.!error.name.length")
				)
			)
		);
		...
		$rules = array(
			...
			'name'=> array(
				'response_empty' => array(
					'rule' => "isEmpty",
					'negate' => true,
					'message' => $this->_("SupportManagerproResponses.!error.name.response_empty")
				),
				'response_length' => array(
					'rule' => array("maxLength", 64),
					'message' => $this->_("SupportManagerproResponses.!error.name.response_length")
				)
			),
			'details' => array(
				'empty' => array(
					'rule' => "isEmpty",
					'negate' => true,
					'message' => $this->_("SupportManagerproResponses.!error.details.empty")
				)
			)
		);
