 ...
 */
class SupportManagerproStaff extends SupportManagerproModel {
	 ...
	 */
	private $serializable_settings = array("mobile_ticket_emails", "ticket_emails");
	...
	public function __construct() {
		...
		Language::loadLang("support_managerpro_staff", null, PLUGINDIR . "support_managerpro" . DS . "language" . DS);
	}
	...
	public function delete($staff_id, $company_id) {
		...
		$this->Record->from("support_staff_schedulespro")->
			leftJoin("support_staff_departmentspro", "support_staff_departmentspro.staff_id", "=", "support_staff_schedulespro.staff_id", false)->
				on("support_staff_settingspro.company_id", "=", "support_staff_schedulespro.company_id", false)->
			leftJoin("support_staff_settingspro", "support_staff_settingspro.staff_id", "=", "support_staff_schedulespro.staff_id", false)->
			where("support_staff_schedulespro.staff_id", "=", $staff_id)->
			where("support_staff_schedulespro.company_id", "=", $company_id)->
			delete(array("support_staff_schedulespro.*", "support_staff_departmentspro.*", "support_staff_settingspro.*"));
		
		// Remove this staff from tickets, but leave the support_repliespro alone
		$vars = array('staff_id' => null);
		$this->Record->where("staff_id", "=", $staff_id)->
			update("support_ticketspro", $vars, array("staff_id"));
	...
	private function updateStaff($staff_id, $company_id, array $vars) {
		...
		$this->addToDepartments($staff_id, $company_id, $this->ifSet($vars['departments'], array()));
		
		// Add the staff schedules
		$this->updateSchedule($staff_id, $company_id, $this->ifSet($vars['schedules'], array()));
		
		// Add the staff settings
		$this->setSettings($staff_id, $company_id, $this->ifSet($vars['settings'], array()));
	...
	private function updateSchedule($staff_id, $company_id, array $schedules) {
		...
		$fields = array("staff_id", "company_id", "day", "start_time", "end_time");
		foreach ($schedules as $schedule) {
			if (!empty($schedule['start_time']) && !empty($schedule['end_time'])) {
				$vars = array_merge(array('staff_id' => $staff_id, 'company_id' => $company_id), $schedule);
				$this->Record->insert("support_staff_schedulespro", $vars, $fields);
			}
		}
	}
	...
	private function addToDepartments($staff_id, $company_id, array $departments) {
		...
		$this->Record->from("support_staff_departmentspro")->
				on("support_departmentspro.company_id", "=", $company_id)->
			innerJoin("support_departmentspro", "support_departmentspro.id", "=", "support_staff_departmentspro.department_id", false)->
			where("support_staff_departmentspro.staff_id", "=", $staff_id)->
			delete(array("support_staff_departmentspro.*"));
		
		// Add the staff to the given departments
		foreach ($departments as $department_id) {
			$vars = array('staff_id' => $staff_id, 'department_id' => $department_id);
			$this->Record->insert("support_staff_departmentspro", $vars);
		}
	}
	 ...
	 *
	 * @param boolean $order True to order the days by the calendar setting, false otherwise
	 * @return array A list of days and their language
	 */
	public function getDays($order = false) {
		$days = array(
			'sun' => $this->_("SupportManagerproStaff.days.sun"),
			'mon' => $this->_("SupportManagerproStaff.days.mon"),
			'tue' => $this->_("SupportManagerproStaff.days.tue"),
			'wed' => $this->_("SupportManagerproStaff.days.wed"),
			'thu' => $this->_("SupportManagerproStaff.days.thu"),
			'fri' => $this->_("SupportManagerproStaff.days.fri"),
			'sat' => $this->_("SupportManagerproStaff.days.sat")
		);
		...
		if ($order) {
			...
			$setting = $this->SettingsCollection->fetchSetting(null, Configure::get("Blesta.company_id"), "calendar_begins");
	...
	public function setSetting($staff_id, $company_id, array $vars) {
		$vars = array_merge($vars, array('staff_id' => $staff_id, 'company_id' => $company_id));
		
		// Update the setting
		if (isset($vars['key'])) {
			$value = $this->ifSet($vars['value']);
			...
			$this->Record->duplicate("value", "=", $this->ifSet($vars['value']))->
				insert("support_staff_settingspro", $vars);
		}
	}
	...
	public function setSettings($staff_id, $company_id, array $settings) {
		...
		foreach ($settings as $key => $value)
			$this->setSetting($staff_id, $company_id, array('key' => $key, 'value' => $value));
	}
	...
	public function getSettings($staff_id, $company_id) {
		...
		$settings = $this->Record->select()->from("support_staff_settingspro")->
	...
	public function get($staff_id, $company_id) {
		...
		$staff = $this->Record->select(array("staff.*", "staff_groups.company_id"))->
			from("staff")->
		...
		if (!$staff)
			$staff = (object)array('id' => $staff_id, 'company_id' => $company_id);
		
		// Get staff departments
		$staff->departments = $this->Record->select("support_departmentspro.*")->
			from("support_staff_departmentspro")->
			innerJoin("support_departmentspro", "support_departmentspro.id", "=", "support_staff_departmentspro.department_id", false)->
			where("support_staff_departmentspro.staff_id", "=", $staff->id)->
			fetchAll();
	...
	public function getSchedules($staff_id, $company_id) {
		return $this->Record->select()->
			from("support_staff_schedulespro")->
			where("staff_id", "=", $staff_id)->
			where("company_id", "=", $company_id)->
			fetchAll();
	}
	 ...
	 */
	public function getAll($company_id, $department_id = null, $get_settings = true, array $order_by = array('first_name' => "asc")) {
	 ...
	 * 	e.g. array('sun' => '08:00:00', 'mon' => '10:00:00') fetches staff available Sunday at 8am UTC AND monday at 10am UTC
	 ...
	 * @return array A list of stdClass objects representing staff
	 */
	public function getList($company_id, $department_id = null, $get_settings = true, $page = 1, array $order_by = array('first_name' => "asc")) {
	...
	private function getStaff($company_id, $department_id = null) {
		// Select staff info
		$this->Record = $this->Record->select(array("staff.*", "staff_groups.company_id"))->
			from("support_staff_schedulespro")->
			leftJoin("staff", "staff.id", "=", "support_staff_schedulespro.staff_id", false)->
		...
		if ($department_id) {
			$this->Record->
					on("support_staff_departmentspro.department_id", "=", $department_id)->
				innerJoin("support_staff_departmentspro", "support_staff_departmentspro.staff_id", "=", "staff.id", false);
		}
		
		return $this->Record->where("support_staff_schedulespro.company_id", "=", $company_id)->
			group("support_staff_schedulespro.staff_id");
	}
		...
		$rules = array(
			'staff_id' => array(
				'exists' => array(
					...
					'message' => $this->_("SupportManagerproStaff.!error.staff_id.exists")
				),
				'belongs' => array(
					'rule' => array(array($this, "validateStaffInCompany"), $this->ifSet($vars['company_id'])),
					'message' => $this->_("SupportManagerproStaff.!error.staff_id.belongs")
				),
				'duplicate' => array(
					'rule' => array(array($this, "validateDuplicateStaff"), $this->ifSet($vars['company_id'])),
					'message' => $this->_("SupportManagerproStaff.!error.staff_id.duplicate")
				)
			),
			'company_id' => array(
				'exists' => array(
					'rule' => array(array($this, "validateExists"), "id", "companies"),
					'message' => $this->_("SupportManagerproStaff.!error.company_id.exists")
				)
			),
			'departments' => array(
				'exists' => array(
					'rule' => array(array($this, "validateDepartmentExists"), $this->ifSet($vars['company_id'])),
					'message' => $this->_("SupportManagerproStaff.!error.departments.exists")
				)
			),
			'schedules' => array(
				'minimum' => array(
					'rule' => array(array($this, "validateScheduleDaysGiven")),
					'message' => $this->_("SupportManagerproStaff.!error.schedules.minimum")
				),
				'unique_days' => array(
					'rule' => array(array($this, "validateScheduleDays")),
					'message' => $this->_("SupportManagerproStaff.!error.schedules.unique_days")
				)
			),
			'schedules[][day]' => array(
				'format' => array(
					'rule' => array(array($this, "validateScheduleDay")),
					'message' => $this->_("SupportManagerproStaff.!error.schedules[][day].format")
				)
			),
			'schedules[][start_time]' => array(
				'format' => array(
					'rule' => array(array($this, "validateScheduleTime")),
					'message' => $this->_("SupportManagerproStaff.!error.schedules[][start_time].format"),
					'post_format' => array(array($this, "dateToUtc"))
				)
			),
			'schedules[][end_time]' => array(
				'format' => array(
					'rule' => array(array($this, "validateScheduleTime")),
					'message' => $this->_("SupportManagerproStaff.!error.schedules[][end_time].format"),
					'post_format' => array(array($this, "dateToUtc"))
				)
			)
		);
	 ...
	 * @param int $staff_id The ID of the staff member
	 * @param int $company_id The ID of the company
	 * @return boolean True if the staff member belongs to the company, false otherwise
	 */
	 ...
	 * @param int $staff_id The ID of the staff member
	 * @param int $company_id The ID of the company
	 * @return boolean True if the staff member does not already exist, false otherwise
	 */
	public function validateDuplicateStaff($staff_id, $company_id) {
		$results = $this->Record->select()->from("support_staff_schedulespro")->
	 ...
	 * @return boolean False if a department given does not exist, true otherwise
	 */
	public function validateDepartmentExists($departments) {
		...
		foreach ($departments as $department_id) {
			if (!$this->validateExists($department_id, "id", "support_departmentspro"))
				return false;
		}
		return true;
	}
	 ...
	 * @return boolean True if no duplicate scheduled days exist, false otherwise
	 */
	 ...
	 *
	 * @param array $schedules A list of staff schedules
	 * @return boolean True if the given scheduled days contains at least one valid day, false otherwise
	 */
	 ...
	 * @return boolean True if the given day is valid, false otherwise
	 */
	 ...
	 * @return boolean True if the time is in a valid format, false otherwise
	 */
