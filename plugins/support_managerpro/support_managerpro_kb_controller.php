 ...
 */
class SupportManagerproKbController extends SupportManagerproController {
	...
	protected function getBreadCrumbs($parent_category_id = null) {
		if (!isset($this->SupportManagerproKbCategories))
			$this->uses(array("SupportManagerproKbCategories"));
		
		$categories = array();
		
		if ($parent_category_id)
			$categories = $this->SupportManagerproKbCategories->getAllParents($parent_category_id);
	...
	protected function getArticleContent($article, $lang = null) {
		if (!isset($this->SupportManagerproKbArticles))
			$this->uses(array("SupportManagerproKbArticles"));
		
		// Fetch the article content if it's not already set
		$all_content = (isset($article->content) ? $article->content : array());
		if (empty($article->content))
			$all_content = $this->SupportManagerproKbArticles->getContent($article->id, $lang);
	...
	protected function getArticleTitleUri($article) {
		...
		$num_chars = (int)Configure::get("SupportManagerpro.max_chars_article_title_uri");
