...
class SupportManagerTickets extends SupportManagerModel {
	...
	public function getStatuses() {
		return array(
			...
			'spam' => $this->_("SupportManagerTickets.status.spam"),
			'deleted' => $this->_("SupportManagerTickets.status.deleted")
		);
	}
	...
	public function SpamStatus($ticket_id, $staff_id = null) {
		...
		$statuses = $this->getStatuses();
		$vars = array(
			'details' => Language::_("SupportManagerTickets.log.status", true, $statuses['spam']),
			'type' => "log"
		);
		// Begin a transaction
		$this->Record->begin();
	...
	public function DeletedStatus($ticket_id, $staff_id = null) {
		...
		$statuses = $this->getStatuses();
		$vars = array(
			'details' => Language::_("SupportManagerTickets.log.status", true, $statuses['deleted']),
			'type' => "log"
		);
		// Begin a transaction
		$this->Record->begin();
	...
	public function PurgeTickets($ticket_id, $staff_id = null) {
		...
		$this->Record->from("support_replies")->
			leftJoin("support_tickets", "support_tickets.id", "=", "support_replies.ticket_id", false)->
				on("support_attachments.reply_id", "=", "support_replies.id", false)->
			leftJoin("support_attachments", "support_attachments.reply_id", "=", "support_attachments.reply_id", false)->
			where("support_tickets.id", "=", $ticket_id)->
			delete(array("support_replies.*", "support_tickets.*", "support_attachments.*"));
	...
	public function PurgeReply($reply_id, $staff_id = null) {
		...
		$this->Record->from("support_replies")->
			...
			leftJoin("support_attachments", "support_attachments.reply_id", "=", "support_attachments.reply_id", false)->
			where("support_replies.id", "=", $reply_id)->
			delete(array("support_replies.*", "support_attachments.*"));
		// Commit the transaction
		$this->Record->commit();
	}
	...
	public function MergeTickets($ticket_id, $ticket_idm, $staff_id = null) {
        ...
        // Merge Tickets
        $this->Record->where("ticket_id", "=", $ticket_id)->update("support_replies", array("ticket_id"=>$ticket_idm));
		$this->Record->commit();
    ...
    **/
	public function getTicketByEmail($email, $get_replies = true, array $reply_types = null) {
		// Get the ticket
        $ntickets = 0;
		$ticket = $this->getTickets()->where("support_tickets.email", "=", $email)->where("support_tickets.status", "=", "spam")->fetch();
        $ntickets = $ticket->id;
        if ($ntickets > 0)
        return $ntickets;
	}
